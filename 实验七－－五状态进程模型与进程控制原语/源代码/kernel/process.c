...
extern void restart_flags();
short int child_stack,fa_stack;
void do_fork(){
	process_num++;
	PCB_queue[ process_num].f_pid = w_is_r;		//child -> father
	PCB_queue[ process_num].process_id = process_num;
	//copy_fa_Tss
	
	update_fa();
	restart_flags();	//to fix some stack bug
	__asm__("pop %cx");
	// update fa end	
	...
	child_stack = (PCB_queue[ process_num].tss.Stack_END-0x200)/16;
